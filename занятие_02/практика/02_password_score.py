"""Генератор паролей. Ветка feature-score: правильная оценка.

Отличия от 01_password_score_raw.py — только внутри check_strength():
оценка по баллам (длина + буквы + цифры + символы), остальной код одинаков.

При merge этой ветки в main возникнет конфликт: обе ветки меняли
одну и ту же функцию. Решаем его в редакторе слияния — Incoming (эта ветка).

Запуск:  python 02_password_score.py
"""
